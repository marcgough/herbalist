...
const phases = [
  phase({
    ...
    checks: [
      ...
      'Production dispatch preflight proves the exact launch commit is release-backed, credential-ready, dispatch-ready, and in the correct DNS-transition or final mode before the guarded workflow is run.',
      ...
      'Cloudflare API token permission requirements are documented and verified without exposing token values.',
      ...
      'Production operator brief consolidates the no-secret launch sequence across dispatch, DNS, feed seed, and strict live verification.',
  ...
  phase({
    ...
    commands: [
      ...
      command('npm run verify:production-deploy-evidence-artifact-content'),
      command('npm run verify:github-release-evidence-content'),
