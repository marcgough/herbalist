...
Capture the next clean Wellcome forward-acquisition lane after the American eclectic, Brazilian, and tables pass completed successfully.

This note starts from the refreshed selector output after the latest three Wellcome books landed. It then prunes the remaining ranked list where it is still dominated by deep lexicon repeats, supplement-only material, thin pharmacopoeia witnesses, administrative pamphlets, article-reprint bundles, or current `404` text-endpoint cases.

## Recommended immediate Wellcome forward slice

1. `wellcome-rnpdxpd2`
   - `The American medical guide for the use of families`
   - Why: uncovered household-reference and materia-medica bridge, directly retrievable through the official Wellcome text route, and better aligned with the practical public-reference spine than another repeat dictionary.

2. `wellcome-cr3ah4dq`
   - `The book of health, or, Thomsonian theory and practice of medicine`
   - Why: directly retrievable broad practice-of-medicine witness with explicit pathology, therapeutics, and materia-medica framing that widens the self-care and household-knowledge lane.

3. `wellcome-bsnmkdkc`
   - `The book of prescriptions`
   - Why: directly retrievable large compendium with prescriptions, dose lists, disease and remedy indexing, and a history-of-materia-medica bridge that makes it a stronger practical reference than the remaining thin or supplement-shaped candidates.
...
- `wellcome-p6afhue8`
  ...
  - Why held back: directly retrievable, but supplement-only framing makes it less foundational than the selected slice.
...
- one household-reference guide first
- one broad book-of-health bridge second
- one large prescriptions compendium third
- direct `200` text-route witnesses before current `404` cases
- no repeat lexicons, supplements, thin pharmacopoeia, administrative disputes, or article-reprint bundles in the immediate slice
