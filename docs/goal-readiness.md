...
`npm run verify:github-release-evidence` checks public GitHub Actions metadata for fresh successful CI and manual release-gate runs on the intended launch commit, then verifies the uploaded visual-smoke and structured release-evidence artifact metadata without downloading artifacts or mutating GitHub. The structured release packet records public Signals item count, topic coverage, source-lane coverage, source-health counts, warning count, source-preservation state, source policy, and Big Pharma blocklist proof.
